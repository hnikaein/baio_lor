...
int *BasketMinHash::get_sketch(const Sequence &sequence, const unsigned int chunk_size, const int gingle_length,
                               const int gingle_gap) const {
    return get_sketch(sequence.seq_str, sequence.size, chunk_size, gingle_length, gingle_gap);
}

int *BasketMinHash::get_sketch(const char *seq_str, const unsigned long seq_str_len, const unsigned int chunk_size,
                               const int gingle_length, const int gingle_gap) const {
    unsigned int sketch_size = chunk_size / sketch_ratio;
    sketch_size = SKETCH_SIZE; //TODO
    ...
    for (int i = 0; i < seq_str_len - gingle_length - gingle_gap; i++)
        for (int j = 0; j <= gingle_gap; j++) {
            int new_hash_temp = new_hash[j] = hash_function(seq_str + i, gingle_length, new_hash[j], MAX_BASENUMBER, j);
    ...
    for (auto i = static_cast<int>(sketch_size - 2); i >= 0; i -= 1)
        if (sketch[i] == max_hash_function)
            sketch[i] = (i + 1) * sketch_step - 1; //sketch[i + 1];
